 ...
 * (2) 使用异步IO来提高访问吞吐量
 *
 * Flink与外部存储系统进行读写操作的时候可以使用同步方式，
 * 也就是发送一个请求后等待外部系统响应，然后再发送第二个读写请求，这样的方式吞吐量比较低，可以用提高并行度的方式来提高吞吐量，
 * 但是并行度多了也就导致了进程数量多了，占用了大量的资源。
 *
...
public class JoinDemo3 {
  ...
  static class JoinDemo3AyncFunction
      extends RichAsyncFunction<Tuple2<String, Integer>, Tuple3<String, Integer, String>> {
    ...
    //异步查询方法
    @Override
    public void asyncInvoke(Tuple2<String, Integer> input, ResultFuture<Tuple3<String, Integer, String>> resultFuture)
        throws Exception {
      // 使用 city id 查询
    ...
    //超时处理
    @Override
    public void timeout(Tuple2<String, Integer> input, ResultFuture<Tuple3<String, Integer, String>> resultFuture)
        throws Exception {
      List list = new ArrayList<Tuple2<Integer, String>>();
