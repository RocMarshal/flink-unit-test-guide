 ...
 * @date 2021/9/13
 * @description 1、 预加载维表
 * 通过定义一个类实现RichMapFunction，在open()中读取维表数据加载到内存中，
 * 在probe流map()方法中与维表数据进行关联。RichMapFunction中open方法里加载维表数据到内存的方式特点如下：
 *
 * 优点：实现简单. 缺点：因为数据存于内存，所以只适合小数据量并且维表数据更新频率不高的情况下。
 * 虽然可以在open中定义一个定时器定时更新维表，但是还是存在维表更新不及时的情况。下面是一个例子：
 */
