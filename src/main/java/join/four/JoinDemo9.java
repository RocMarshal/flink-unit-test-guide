 ...
 * Temporal table 是持续变化表上某一时刻的视图，
 * Temporal table function是一个表函数，传递一个时间参数，返回Temporal table这一指定时刻的视图。
 *
 * 可以将维度数据流映射为Temporal table，
 * 主流与这个Temporal table进行关联，可以关联到某一个版本（历史上某一个时刻）的维度数据。
 *
 * Temporal table function join的特点如下：
 *
 * 优点：维度数据量可以很大，维度数据更新及时，不依赖外部存储，
 * 可以关联不同版本的维度数据。
